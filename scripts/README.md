...
| `check-connector-spec-help.cjs` | Validates `helpKey` and `sectionHelpMessage` length/link rules in `connector-spec.json` (runs in `npm run lint`). |
...
| `slim-connector-spec-help.cjs` | Maintainer script to shorten `helpKey` and `sectionHelpMessage` with links to Configuration reference and Use guides. |
