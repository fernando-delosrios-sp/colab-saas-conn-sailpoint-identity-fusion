...
2.  **Global Pre-processing**:
    ...
    - Bulk-registers unique attribute values directly from managed source account data (`attributes.registerUniqueValuesFromManagedSourceAccounts`) to build the collision registry used by the post-rebuild refresh in step 3. This is a lightweight path that does not hydrate full `FusionAccount` instances.
