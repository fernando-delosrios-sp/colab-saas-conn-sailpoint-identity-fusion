...
7.  **Form & Entitlement Reconciliation**:
    ...
    !!! warning "Upgrade note: Fusion review form definitions"
        ...
        - **The update:** Candidate identities receive their status from data stored on pending form instances. The connector now declares a `candidates` field on the form definition so that value round-trips from ISC across aggregations.
