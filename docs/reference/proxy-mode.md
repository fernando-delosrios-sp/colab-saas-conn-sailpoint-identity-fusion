...
Set the same password in ISC (**Proxy password**) and on the proxy server (`PROXY_PASSWORD` environment variable).
...
1. Deploy server to an accessible endpoint (e.g., `https://fusion-proxy.company.com/fusion`)
2. In ISC connector, enable proxy mode and set Proxy URL and password (see [Advanced configuration](../configuration/advanced.md))
...
| **Client config**   | Enable proxy mode; set Proxy URL and password in ISC ([field reference](../configuration/advanced.md)) |
