...
| **$accounts**                 | Managed account snapshots: source **`attributes`** plus **`_id`**, nested **`source`** (`id`, `name` — managed accounts only for `id`), nested **`schema`** (`id` = native identity / account key, `name` = display name), and **`IIQDisabled`**. Ordered by configured source order, then account insertion order, then unknown sources. If `mainAccount` contains a valid managed account ID, that account is moved to index 0. | `$accounts[0]._id`, `$accounts[0].source.name`, `$accounts[0].schema.name` |
...
| **$account**                  | Snapshot for the origin only: same shape as `$accounts[]` entries when the origin is a managed account; when the origin is **Identities** and identity attributes are available, identity-backed fields are used first (then synthetic identity row if needed). **`source.name`** is **`Identities`** for identity-backed rows (no `source.id`). **`schema`** carries the display name and id for identity-backed rows. | `$account.schema.name`, `$account.source.name`                |
