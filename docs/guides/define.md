...
| **$accounts**                 | Managed account snapshots available to templates. Ordered by configured source order, then account insertion order, then unknown sources. If `mainAccount` contains a valid managed account ID, that account is moved to index 0. | `$accounts[0]._source`, `$accounts[0].IIQDisabled` |
