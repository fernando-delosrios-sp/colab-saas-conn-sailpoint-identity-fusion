...
| **$accounts**                 | Managed account snapshots: source **`attributes`** plus **`managedAccountKey`** (composite `sourceId::nativeIdentity`), **`_name`**, **`_source`**, **`_sourceId`**, **`_nativeIdentity`**, optional **`_rawId`**, **`IIQDisabled`**. Ordered by configured source order, then account insertion order, then unknown sources. If `mainAccount` contains a valid managed account key, that account is moved to index 0. | `$accounts[0].managedAccountKey`, `$accounts[0]._name`, `$accounts[0]._source` |
...
| **$account**                  | Snapshot for the origin only: same shape as `$accounts[]` entries when the origin is a managed account; when the origin is **Identities** and identity attributes are available, identity-backed fields are used first (then synthetic identity row if needed). Managed rows include **`managedAccountKey`**; identity-backed rows include **`originIdentityId`**. Both include **`_name`** and **`_source`**. | `$account.managedAccountKey`, `$account._name`, `$account._source`                |
