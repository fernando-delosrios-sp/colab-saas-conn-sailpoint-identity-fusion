...
| **$accounts**                 | Managed account snapshots: source **`attributes`** plus **`_id`**, **`_name`** (from the account’s top-level id and name), **`_source`**, **`IIQDisabled`**. Ordered by configured source order, then account insertion order, then unknown sources. If `mainAccount` contains a valid managed account ID, that account is moved to index 0. | `$accounts[0]._id`, `$accounts[0]._name`, `$accounts[0]._source` |
