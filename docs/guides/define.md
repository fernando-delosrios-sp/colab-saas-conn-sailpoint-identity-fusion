...
| **$accounts**                 | Managed account snapshots: source **`attributes`** plus **`_name`**, **`_source`**, **`_sourceId`**, **`_nativeIdentity`**, optional **`_rawId`**, **`IIQDisabled`**. Composite key = `sourceId::nativeIdentity` is **`$originAccount`** for the origin only, not a field on every snapshot. Ordered by configured source order, then account insertion order, then unknown sources. If `mainAccount` contains a valid managed account key, that account is moved to index 0. | `$accounts[0]._sourceId`, `$accounts[0]._nativeIdentity`, `$accounts[0]._name` |
...
| **$account**                  | Snapshot for the origin only: same shape as `$accounts[]` entries when the origin is a managed account; when the origin is **Identities** and identity attributes are available, identity-backed fields are used first (then synthetic identity row if needed). Use **`$originAccount`** for the managed composite key or identity id string. Identity-backed rows include **`originIdentityId`**. | `$originAccount`, `$account._name`, `$account._source`                |
