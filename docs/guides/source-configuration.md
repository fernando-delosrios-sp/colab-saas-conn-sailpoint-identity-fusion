...
| **Delete accounts with no managed accounts left?** | Remove Fusion accounts when they have no contributing source accounts (and, for identity-origin accounts, when their origin identity is no longer in the configured identity scope) | No       | Useful for automated cleanup when users leave. Identity-origin accounts retain their `baseline` status when removed |
