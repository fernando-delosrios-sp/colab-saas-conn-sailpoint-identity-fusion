...
!!! note "Deferred matching"
    ...
    **Deferred matching** compares an unmatched account to other new unmatched accounts from the same source in the same aggregation run. If the best match is such a same-source peer, the connector defers creating a new Fusion identity until a later aggregation, when that peer is part of the established baseline. Cross-source peer checks are never performed. Disable this option when one person may appear as multiple accounts in a single aggregation and you want each account evaluated independently.
