...
| **Include record accounts in Match**   | Match scoring for record sources                                  | No (only for Records)       | Default on. When off, identity and deferred-candidate scoring are skipped; Map & Define and unique-attribute registration still execute (for example to reserve third-party identifiers without matching).                                                                                                        |
...
| **Deferred candidate matching**        | Optional comparison against deferred candidates within the same operation | No (only for Authoritative) | When enabled, non-matched accounts are compared against other deferred candidates from the same source in the same operation. If the only match is a same-source deferred candidate, identity creation is deferred. Cross-source checks are never performed.                                               |
