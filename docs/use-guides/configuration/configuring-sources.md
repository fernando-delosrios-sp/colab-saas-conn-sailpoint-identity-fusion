...
| **Include record accounts in Match**   | Match scoring for record sources                                  | No (only for Records)       | Default on. When off, identity and deferred-candidate scoring are skipped; a bulk **record unique registration** step runs selective Map plus unique-value registration (passthrough or coincident attribute maps only) before the uncorrelated match sweep. Normal Define and full match processing are not run for those accounts. |
...
| **Deferred candidate matching**        | Optional comparison against deferred candidates within the same run | No (only for Authoritative) | When enabled, non-matched accounts are compared against other deferred candidates from the same source in the same run. If the only match is a same-source deferred candidate, identity creation is deferred. Cross-source checks are never performed.                                               |
...
| **Aggregation batch size**             | Maximum accounts to aggregate per aggregation                         | No                          | Leave empty for all accounts; useful for initial loading of datasets.                                                                                                                                                                                                                                    |
...
| **Correlation mode**                   | How to handle missing source accounts                                 | Yes                         | Options: **Correlate missing accounts on aggregation** (direct API patch), **Reverse correlation from managed source** (sets an attribute for ISC native correlation), **Do not correlate** (none). See [Managing correlation](managing-correlation.md).                                                                                                      |
...
!!! note "Deferred candidates"

    **Deferred candidate matching** compares a non-matched account to other deferred candidates from the same source in the same run. If the best match is such a same-source deferred candidate, the connector defers creating a new Fusion identity until a later aggregation, when that deferred candidate is part of the established baseline. Cross-source deferred-candidate checks are never performed. Disable this option when one person may appear as multiple accounts in a single aggregation and you want each account evaluated independently.
...
!!! note "Cross-guide note"
    ...
    If you keep **Include record accounts in Match** enabled, record sources participate in Match scoring using your global Match rules. See [Match guide](matching-identities.md) for threshold, mandatory rule, and skip-if-missing tuning.
...
!!! note
    ...
    **Force attribute refresh on next aggregation?** is located at **Advanced Settings → Developer Settings**. It forces Normal-type attributes to refresh on the next aggregation only, after which the option is automatically turned off. Applies only to Normal attributes; Unique attributes are only computed when a Fusion account is first created or when an existing account is activated. Can be expensive for large datasets.
