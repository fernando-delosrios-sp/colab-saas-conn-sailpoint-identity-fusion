...
| **Decision log segment** | Compact run counter in logs: `decisions(Nn/Mm/NMnm/Aa)` where `n` = new identity, `m` = merge, `nm` = no-match (record/orphan), `a` = auto-merge. Appears in `STATUS`, `DETAIL`, `EVENT_SUMMARY`, and phase-complete lines. See [Connection and observability tuning — Operation log line kinds](use-guides/operation/connection-and-observability-tuning.md#operation-log-line-kinds-accountlist). |
