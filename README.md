...
> **Note:** In Velocity context, managed account snapshots (`$accounts` and `$sources`) include each source account’s `**attributes`** plus `**managedAccountKey**` (same composite as `sourceId::nativeIdentity`), `**_name**` (from the ISC account’s top-level `name` / `nativeIdentity`), `**_source**`, `**_sourceId**`, `**_nativeIdentity**`, optional `**_rawId**`, and `**IIQDisabled**` (IdentityIQ-style disabled flag where `true` means disabled). `$accounts` is deterministic: sources follow configured order, accounts keep insertion order within each source, and non-configured sources are appended. `**$originAccount**` is the immutable origin key string; `**$account**` is the origin snapshot (managed shape or identity-backed when the origin is `Identities`).
