...
| **Automatically assign on match score threshold?** | Assign to the matched identity without review when the combined match score meets or exceeds the automatic assignment threshold | No       | If enabled, requires setting the **Automatic assignment match score**. Matches meeting this threshold receive an **AUTO** badge and skip manual review; otherwise they receive a **MANUAL** badge. |
| **Automatic assignment match score [0-100]** | Minimum combined score required to trigger automatic assignment | Yes (if enabled) | Defines the threshold for the AUTO badge. |
...
| **Skip match if missing**                 | Skip rule when one side is missing             | No       | Default: Yes. Skipped rules do not affect the combined score.
...
- `includeExact` (boolean): Emit rows that have at least one **auto** match candidate: the combined match score meets or exceeds the **Automatic assignment match score**. You can use this without `includeMatched` to list only those rows. (Note: this parameter historically matched exact rules, but now corresponds to the automatic assignment threshold.)
