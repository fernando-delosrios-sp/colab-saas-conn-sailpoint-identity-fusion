...
> **Note:** In Velocity context, managed account snapshots (`$accounts` and `$sources`) include each source account’s `**attributes`** plus `**_name**` (from the ISC account’s top-level `name` / `nativeIdentity`), `**_source**`, `**_sourceId**`, `**_nativeIdentity**`, optional `**_rawId**` (ISC account id), and `**IIQDisabled**` (IdentityIQ-style disabled flag where `true` means disabled). The composite managed key (`sourceId::nativeIdentity`) is `**$originAccount**` for the originating row only — it is not duplicated on every snapshot. `$accounts` is deterministic: sources follow configured order, accounts keep insertion order within each source, and non-configured sources are appended. `**$account**` is the origin snapshot (managed shape or identity-backed when the origin is `Identities`).
...
| **Maximum candidates per review form** | Limit of potential matches shown on review form      | No                                          | Limits how many highest-scoring identity candidates are included on manual review forms. Valid range 1-15. Default: 10. |
