...
## 2026-05-27 - Bare URLs in MkDocs tables trigger linting errors

**Learning:** MkDocs markdown linting (specifically the MD034 rule for "Bare URLs used") frequently flags example email addresses within markdown tables if they aren't enclosed in backticks or explicitly formatted as links.
**Action:** When updating or reviewing documentation with example emails or URLs in tables, always wrap them in backticks to treat them as code block literals. This prevents them from erroneously rendering as clickable `mailto:` links and ensures the markdown linter passes successfully.
