...
## 2026-06-02 - Refactor dense warnings inside admonitions
**Learning:** Dense paragraphs inside admonitions (like the upgrade warning in `docs/operations/account-list.md`) can bury the necessary context, impact, and solution.
**Action:** When clarifying dense paragraphs inside MkDocs admonitions, refactor the wall of text into a bulleted list with bolded lead-in prefixes (e.g., `- **The update:**`, `- **The impact:**`). Ensure proper nested indentation (e.g., 8 spaces if the parent admonition is indented at 4 spaces and is part of a list itself).
