...
## 2026-07-08 - Prevent Array.from Iterator Heap Allocation
**Learning:** Iterating over `Map.values()` using `Array.from(map.values()).some(...)` creates unnecessary intermediate arrays, leading to heap allocations and garbage collection overhead, especially in hot loops like `mapAttributes`.
**Action:** Replace `Array.from(map.values()).some(...)` with a direct `for...of` loop over the `Map`'s iterator to prevent array allocations and maintain the same short-circuiting logic without the memory overhead.
