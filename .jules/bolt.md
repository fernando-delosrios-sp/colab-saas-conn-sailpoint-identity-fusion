...
## 2026-07-06 - Avoid Array.from(map.values()).some for iteration

**Learning:** Calling `Array.from(map.values())` just to iterate over the items using `.some()` eagerly allocates a new array in memory before performing the iteration. This creates unnecessary heap allocations and garbage collection overhead, particularly in hot loops.
**Action:** Iterate directly over `map.values()` using a `for...of` loop and manually implement the short-circuiting logic (`break`).
