...
## 2026-07-16 - Prevent Heap Allocations in Hot Loops

**Learning:** Iterating over `Map` or `Set` objects using `Array.from(collection.values()).some(...)` creates unnecessary intermediate arrays, leading to heap allocations and garbage collection overhead, especially in hot loops like `refreshUniqueAttributes`.
**Action:** Replace `Array.from(map.values()).some(...)` with a direct `for...of` loop over the map's iterator to prevent allocations and maintain the same short-circuiting logic without the memory overhead.
