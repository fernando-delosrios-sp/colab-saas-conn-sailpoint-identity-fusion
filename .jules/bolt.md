...
## 2026-05-24 - Avoid Array.from(set/map) chaining for iteration
**Learning:** Calling `Array.from(map.values())` or `Array.from(set)` just to iterate over the items (e.g., via `for...of` or `.map()`, `.filter()`, `.some()`, `.find()`) is an anti-pattern that creates unnecessary intermediate arrays and heap allocations, hurting performance in hot paths.
**Action:** Instead of converting the Map/Set to an Array, iterate over it directly using a `for...of` loop or use dedicated iterators.
