...
## 2026-06-26 - Prevent Heap Allocations in Hot Paths

**Learning:** Iterating over `Map` objects using `Array.from(map.values()).some(...)` or `Array.from(map.entries()).filter(...)` creates unnecessary intermediate arrays, leading to heap allocations and garbage collection overhead, especially in hot loops like `mapAttributes` and `initializeCounters`.
**Action:** Replace `Array.from` calls with direct `for...of` loops to prevent allocations and maintain the same logic without the memory overhead.
