...
## 2026-06-01 - Avoid Array.from(map.values()) for iteration

**Learning:** Iterating over `Map` values using `Array.from(map.values()).some(...)` creates unnecessary intermediate arrays, leading to heap allocations and garbage collection overhead, especially in hot loops like in `attributeService.mapAttributes`.
**Action:** Replace `Array.from(map.values()).some(...)` with a direct `for...of` loop over the `Map` iterator to prevent allocations and maintain the same short-circuiting logic without the memory overhead.
