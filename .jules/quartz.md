...
## 2026-08-06 - Consolidating Ternary Returns for Clarity

**Learning:** Duplicating ternary returns across `if/else` branches (e.g. returning `'deferred-match' : 'non-match'`) makes the logic harder to scan.
**Action:** Extract the differing conditional evaluation into a single derived boolean variable (e.g. `const hasMatch`) and consolidate the return into one clean ternary statement.
