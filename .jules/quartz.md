...
## 2026-06-27 - Flattening nested functional property access
**Learning:** Using chained functional helpers like `readUnknown(readUnknown(obj, 'propA'), 'propB')` is hard to read and scan.
**Action:** Replace nested `readUnknown` calls with the existing `readPathUnknown(obj, ['propA', 'propB'])` utility to flatten property access and improve readability.
