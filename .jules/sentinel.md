...
## 2026-06-17 - SSRF Vulnerability via URL Parsing Bypass
**Vulnerability:** URL protocol validation relied on `.toLowerCase().startsWith('http://')` and `.toLowerCase().startsWith('https://')` for both proxy URLs and external logging URLs.
**Learning:** Checking URL validity using `.startsWith()` is insufficient because string prefixes can be trivially bypassed with malformed URLs or unexpected payloads (e.g., `http://malicious.com`).
**Prevention:** Always use the built-in `new URL()` constructor to correctly parse URLs and explicitly check the parsed `.protocol` property (e.g., `parsedUrl.protocol === 'http:' || parsedUrl.protocol === 'https:'`) to prevent Server-Side Request Forgery (SSRF) bypasses.
