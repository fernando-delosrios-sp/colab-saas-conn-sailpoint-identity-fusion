...
## 2026-06-28 - Insecure URL Prefix Validation (SSRF Risk)
**Vulnerability:** Multiple services (`proxyService`, `logService`, and settings validators) used `.toLowerCase().startsWith('http://')` to validate configured URLs. This approach is vulnerable to bypass via malformed URLs like `\thttp://example.com` or `http:file:///etc/passwd`, leading to Server-Side Request Forgery (SSRF) or Local File Inclusion.
**Learning:** String prefix checks are insufficient for URL validation, especially in Node.js where the `URL` constructor is forgiving of malformed schemes.
**Prevention:** Always use the built-in `URL` class for validation combined with explicit scheme checks, whitespace rejection (`url.trim() !== url`), and strict prefix verification (`url.toLowerCase().startsWith(\`\${parsed.protocol}//\`)`). Created `isValidHttpUrl` utility for centralized robust validation.
