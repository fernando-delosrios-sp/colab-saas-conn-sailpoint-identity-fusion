...
**Learning:** External or user-provided URLs must always be validated prior to making network requests, especially in Node.js where `fetch` or HTTP clients might attempt to resolve arbitrary schemes or hostnames.
**Prevention:** Enforce strict URL scheme validation (e.g., checking for `http://` or `https://`) whenever initializing requests with configured URLs.
## 2026-07-02 - SSRF Protocol Bypass
**Vulnerability:** URL validation using only string prefix checks like `.toLowerCase().startsWith('http://')` can be bypassed with malformed payloads.
**Learning:** Node's `URL` parser is forgiving, and prefix checks allow whitespace injection. Strict parsing and explicit protocol prefix checks are required.
**Prevention:** Use `isValidHttpUrl` with strict parsing, whitespace rejection, and exact scheme-slash matching.
