...
## 2026-07-27 - URL Parsing for SSRF Defense

**Vulnerability:** Vulnerable URL prefix checking (`startsWith('http://')`) was used instead of proper URL parsing.
**Learning:** Checking prefixes is insufficient to prevent SSRF because attackers can construct URLs that pass the prefix check but resolve to unintended hosts (e.g., `http://malicious.com`).
**Prevention:** Always use the `URL` constructor to parse URLs and then perform strict validation on specific properties like `protocol` and `hostname`.
