...
- [x] 1.2 Render candidate HTML with columns attribute, algorithm, threshold, value, score for all candidates in one block (D3; scenario: Candidate score table matches the review email columns).
...
- [x] 3.1 Extend `deleteExistingForms` (and Setup) so matching Fusion review form definitions are deleted and leftover instances are cancelled or deleted in the same pass (D6; scenario: resetForms only closes in-flight reviews and continues).
...
- [x] 5.3 Add glossary rows for Fusion review form definition, DESCRIPTION HTML, and in-flight review in `openspec/specs/ubiquitous-language/spec.md` (canonical) and `docs/glossary.md`.
