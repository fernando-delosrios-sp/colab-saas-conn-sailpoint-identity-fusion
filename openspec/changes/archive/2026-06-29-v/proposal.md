...
## Follow-up Scope
Task 3.3 (replacing 1196 `any` types across 93 files) was punted from this change. Each `any → unknown` conversion requires per-site type narrowing that cannot be done mechanically without breaking the build. A dedicated follow-up change should iterate file-by-file (starting with public APIs in `src/utils/`), introducing typed interfaces and generics where appropriate.
