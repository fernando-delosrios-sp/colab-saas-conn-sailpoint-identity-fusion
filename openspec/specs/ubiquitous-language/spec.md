...
| **Fusion review form definition** | The ISC Custom Forms definition named from `fusionFormNamePattern` plus the managed account’s identity. It is the template for that account’s Fusion review instance(s). Not a single shared definition for all reviews. |
