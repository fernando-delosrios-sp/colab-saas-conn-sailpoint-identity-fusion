# attribute-service Spec

## Purpose

The attribute service (`src/services/attributeService/attributeService.ts`) renders, validates, and persists Fusion account attributes from Apache Velocity expressions. It exposes a curated Velocity context (mapped attributes, identity/account snapshots, and helper objects) and applies canonical output transforms (`trim` → `case` → `spaces` → `normalize` → counter-aware `maxLength`). This spec defines the contract between the connector, the Velocity templates configured by integrators, and the documentation surfaces that describe them.
## Requirements
...
For identity-origin Fusion accounts, the root identity name SHALL be accessible in the Velocity context as `$identity.name` and SHALL fall back to `$name` when no mapped attribute named `name` exists.

Feature: Fusion account attribute resolution
Rule: For identity-origin Fusion accounts, the root identity name SHALL be available as `$identity.name` and SHALL fall back to `$name` when no mapped attribute named `name` exists.

#### Scenario: `$identity.name` resolves to root identity name
- **GIVEN** an identity-origin Fusion account built from an identity with `name: "Ada Wong"`
...
- **GIVEN** an identity-origin Fusion account whose identity bag contains `{ name: "Attributes Name" }`
- **WHEN** a Velocity expression references `$identity.name`
...
- **GIVEN** an identity-origin Fusion account with no mapped attribute named `name`
...
- **GIVEN** an identity-origin Fusion account with a mapped attribute `name: "Mapped Name"`
- **WHEN** a Velocity expression references `$name`
