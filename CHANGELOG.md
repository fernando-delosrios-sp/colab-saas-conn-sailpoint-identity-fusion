...
- (2026-07-23) **Performance:** Identity-sweep non-match comparisons no longer allocate `ScoreReport[]` breakdown arrays. `compareFusionAccounts` uses a fast combined-score-only path when report capture is off; threshold-passing matches re-run once with full breakdown (matches are rare). Deferred candidates and report-capture runs unchanged. Wired via `MatchingService.setCaptureBreakdown()` from `FusionService.initializeManagedAccountProcessing`. LIG3 upper-bound skip logic colocated in `scoringHelpers` (`lig3UpperBound`, `lig3UpperBoundSkipIfUnreachable`).
