...
- (2026-07-22) **Fix:** Routed the Fusion display attribute override (`fusionDisplayAttribute`) to the authoritative **identity alias** (SDK top-level `displayName`) instead of the former `identityDisplayName` value, eliminating stale persisted labels and ensuring freshly-serialized Fusion accounts reflect the current identity.
- (2026-07-22) **Architecture:** Added `hydrateCorrelatedManagedAccountIdentities` in `src/operations/helpers/corePipeline.ts`, invoked after `processFetchedFormData` and before aggregation, to ensure the identity correlated to each fetched managed account is present in the run-scoped identity cache and its identity layer is applied to the corresponding Fusion account before `getISCAccount` serializes it. Batches the hydration query in chunks of 50 ids with per-chunk error isolation.
- (2026-07-22) **Terminology:** Distinguished **identity alias** (authoritative, used for `fusionDisplayAttribute` and lookup), **identity name** (human-friendly, used in reports/review forms/emails/logs), and **Fusion account name** (`state.name`, internal/conflict tracking) in `openspec/specs/ubiquitous-language/spec.md` and `docs/concepts/glossary.md`. Retired `identity display name` / `identityDisplayName` as a term; `FusionAccount.identityDisplayName` is now an alias for `FusionAccount.identityAlias`.
- (2026-07-22) **Spec:** Added `Requirement: Correlated identities are hydrated before the managed-account sweep` to `openspec/specs/fusion-run/spec.md` covering scope-outs, multi-account sharing, protected-identity skip, no-correlation no-op, and 50-id chunking.
