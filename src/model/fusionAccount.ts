...
import type { FusionAttributeBag, FusionManagedAccountInfo } from './fusionAccountTypes'
import {
...
export class FusionAccount {
    ...
    /** Correlated identity label (matching, forms, attribute pipeline). */
    ...
    private pruneDeletedManagedAccounts(allAccountsById: Map<string, Account>): void {
        ...
        for (const accountId of trackedIds) {
            ...
            if (removedFromAccounts || removedFromMissing) {
                ...
                this.addHistory(
                    `Removed managed account reference (not present in source inventory or excluded by source filters): ${accountId}`
                )
            }
