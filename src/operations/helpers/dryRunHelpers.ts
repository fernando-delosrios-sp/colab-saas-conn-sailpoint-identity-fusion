...
export const refreshUniqueAttributesForDryRun = async (
    ...
    const getAccountSortKey = (obj: any) =>
        String(
            readUnknown(obj, 'originAccountId') ??
                readPathString(obj, ['attributes', 'originAccount']) ??
                readUnknown(obj, 'nativeIdentity') ??
                readUnknown(obj, 'key') ??
                readUnknown(obj, 'name') ??
                ''
        )
    const stableAnalyzed = [...analyzedUncorrelatedAccounts].sort((a: any, b: any) => {
        const aKey = getAccountSortKey(a)
        const bKey = getAccountSortKey(b)
