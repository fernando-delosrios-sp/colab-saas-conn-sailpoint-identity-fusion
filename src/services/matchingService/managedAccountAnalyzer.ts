...
import { hasIdentityCandidateMatches as checkHasIdentityCandidateMatches } from '../fusionService/helpers'
...
export class ManagedAccountAnalyzer {
    ...
    public async scoreIdentityCandidates(account: Account): Promise<ManagedAccountAnalysisContext> {
        ...
        if (recordMatchingEnabled) {
            ...
            const candidateSet = this.state.matchingService.getCandidates(fusionAccount, excludeIds)
            ...
            fusionIdentityComparisons = await this.state.matchingService.scoreFusionAccount(
                fusionAccount,
                identityPool,
    ...
    public async scoreDeferredCandidates(analysis: ManagedAccountAnalysisContext): Promise<void> {
        ...
        analysis.fusionIdentityComparisons += await this.state.matchingService.scoreFusionAccount(
