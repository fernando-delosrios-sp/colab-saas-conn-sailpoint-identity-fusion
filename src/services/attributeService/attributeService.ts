...
export class AttributeService {
    ...
    public async initializeCounters(): Promise<void> {
        ...
        // Avoids O(N) intermediate array generation and garbage collection.
        const existingCounters: Record<string, number> = {}
        for (const [key, value] of stateWrapper.state.entries()) {
            if (typeof value === 'number' && counterDefinitions.some((definition) => definition.name === key)) {
