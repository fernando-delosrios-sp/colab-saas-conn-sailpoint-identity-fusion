...
export async function aggregateManagedSources(deps: SourceAggregatorDeps): Promise<void> {
    ...
    // to prevent memory spikes and API rate limit exhaustion when aggregating multiple sources
...
export async function aggregateDelayedSources(
    ...
    // ⚡ Bolt: Replace unbounded Promise.all mapping with bounded promiseAllBatched
    // to prevent memory spikes and API rate limit exhaustion when scheduling delayed aggregations
    await promiseAllBatched(
        delayedSources,
        async (source) => {
